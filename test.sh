...
if [[ -n $nvim && -s $ROOT/nvim/lazy-lock.json ]]; then
  "$nvim" --headless \
    ...
    "+lua local expected = { n = 'j', e = 'k', N = '}', E = '{', k = 'n', K = 'N', ['<C-n>'] = '<C-D>', ['<C-e>'] = '<C-U>' }; for lhs, rhs in pairs(expected) do if vim.fn.maparg(lhs, 'n') ~= rhs then os.exit(1) end end; for _, lhs in ipairs({ '<C-h>', '<C-n>', '<C-e>', '<C-i>', 'dh' }) do if vim.fn.maparg(lhs, 'i') ~= '' then os.exit(1) end end; if vim.fn.maparg('<C-h>', 'n') ~= '<C-W>h' or vim.fn.maparg('<C-i>', 'n') ~= '' or vim.fn.maparg('e', 'o') ~= '' or vim.fn.maparg('n', 'o') == 'j' or vim.fn.maparg('k', 'o') ~= '' then os.exit(1) end" \
